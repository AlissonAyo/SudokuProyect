...
const int windowSize = gridSize + 100;
...
void updateLives() {
    InvalidateRect(hwndMain, NULL, TRUE); // Forzar repintado para actualizar corazones
}

void showGameOver() {
    MessageBoxA(hwndMain, "Has perdido todas tus vidas. ¡Juego Terminado!", "Game Over", MB_OK);
    ...
    switch (uMsg) {
        ...
        case WM_KEYDOWN: {
            if (gameStarted && selectedCell.x != -1 && selectedCell.y != -1 && wParam >= '1' && wParam <= '9') {
                int num = wParam - '0';
                if (board[selectedCell.y][selectedCell.x] == 0) {  // Solo permitir entrada en celdas vacías
                    if (isSafe(board, selectedCell.y, selectedCell.x, num)) {
                        ...
                        if (completed) {
                            int secondsElapsed = int(double(clock() - startTime) / CLOCKS_PER_SEC);
                            ostringstream oss;
                            oss << "¡Felicidades! Completaste el Sudoku en " << secondsElapsed << " segundos.";
                            MessageBoxA(hwnd, oss.str().c_str(), "Juego Completado", MB_OK);
                            DestroyWindow(hwnd);
                        }
                    } else {
                        lives--;
